...
It may only be used with specific checkers that are not enabled by default, and which are prepared to be enabled this way. Only has effect if '-Xverify-ir' is not 'none'.""",
    ...
    @Argument(
        ...
        description = "Add (+) or remove (-) a callable by its fully qualified name to/from the allowlist of functions whose functional argument may let a captured variable escape.",
    )
